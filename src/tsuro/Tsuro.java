...
public class Tsuro extends JFrame implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if(isGameEnd() || !isMoreMoves() ){
            endGameScreen();
            return;
        }
        //checks if the current board button is legal, and that the current player's button is not null
        if (this.isLegal() && getCurrentPlayerSelectedButton() != null) {
            //sets the connections of the current board button to the connections of the desired button in the players hand
            b.setConnections(getCurrentPlayerSelectedButton().getConnections());
            getCurrentPlayerSelectedButton().setBackground(Color.WHITE);
            getCurrentPlayerSelectedButton().setConnections(TsuroButton.makeRandomConnectArray());
            //true if it is not the current player's first move
            final Player currentPlayer = getCurrentPlayer();
            if (currentPlayer.getMove() == 0) {
                //sets the stone button location to the button on the board
                currentPlayer.setStoneButtonLocation(b);
                b.addStone(currentPlayer.getStoneColor(), currentPlayer.getStoneLocation());
                currentPlayer.removeStoneFromHand();
                currentPlayer.addStoneToHand();
                //increments the number of moves the player has done
                currentPlayer.setMove(currentPlayer.getMove() + 1);
            } else {
                //increments the number of moves the player has done
                currentPlayer.setMove(currentPlayer.getMove() + 1);
                //removes stone from it's previous location
                currentPlayer.getStoneButtonLocation().removeStone(currentPlayer.getStoneLocation());
            ...
            if (currentPlayer.getPlayerNumber() < getNumberPlayers()) {
                adjustPlayers();
    ...
    private TsuroButton getCurrentPlayerSelectedButton() {
        return getCurrentPlayer().getCurrentPlayerButton();
    }
    ...
    private void endGameScreen() {
        if (isGameEnd()) {
            ...
            StringBuilder endMessage = new StringBuilder("Game Over \n");
            // Builds end game message to announce winner
            if (winner != null) {
                endMessage
                        .append("Player ")
                        .append(winner.getPlayerNumber())
                        .append(" Wins");
            } else {
                endMessage.append("Game ended in a Tie!");
            }
            JLabel endGameLabel = new JLabel(endMessage.toString());
            endGameLabel.setFont(new Font("Serif", Font.CENTER_BASELINE, 30));

            this.setSize(column * 110, row * 110);
            this.setLocation(600, 100);
            this.add(endGameLabel, BorderLayout.CENTER);
            this.setVisible(true);
            setGameEnd(true);
        }
    }
    ...
    /**
     * Finds and returns the player that won the game
     *
     * @return the winning player
     */
    private Tsuro.Player getWinner() {
    ...
    private boolean isMoreMoves() {
        ...
        return (counter != getNumberPlayers() - 1);
    }
     ...
     */
    private boolean isCrash(Tsuro.Player playerCheck) {
     ...
     */
    private Tsuro.Player adjustPlayers() {
     ...
     */
    private int getJValue(TsuroButton button) {
    ...
    public boolean isLegal() {
        // the current button selected on the board
        TsuroButton currentBoardButton = getCurrentBoardButton();
        //holds the column value of the desired spot on the board
        int holdI = getIValue(currentBoardButton);
        //holds the row value of the desired spot on the board
        int holdJ = getJValue(currentBoardButton);
        //holds if the action is legal
        boolean indicator = false;
        //checks if the desired spot on the board is empty
        Player currentPlayer = getCurrentPlayer();
        if (currentBoardButton.getConnections() == null && currentPlayer != null) {
    ...
    public boolean isWallCrash(Player player) {
        ...
        switch (player.getStoneLocation()) {
            case 0:
            case 1:
                playerDead = iButton == 1;
                break;
            case 2:
            case 3:
                playerDead = jButton == getColumn();
                break;
            case 4:
            case 5:
                playerDead = iButton == getRow();
                break;
            case 6:
            case 7:
                playerDead = jButton == 1;
                break;
        }
        currentPlayer.setDead(playerDead);
        return playerDead;
    }
     ...
     *
     * @param connect the connection which must be rotated
    ...
    private void moveNext() {
        ...
        if (nextButton != null) {
            ...
            moveStoneToNextButton(nextButton, nextLocation);
            determineCrash();
        }
    }

    private void moveStoneToNextButton(TsuroButton nextButton, int i) {
        //add stone to next button
        nextButton.addStone(getCurrentPlayer().getStoneColor(), nextButton.getConnections()[i]);
        //update stone location
        getCurrentPlayer().setStoneLocation((nextButton.getConnections()[i]));
     ...
     */
    private TsuroButton findNextButton() {
